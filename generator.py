...
class SPADEGenerator(Chain):
    def __init__(self, opt):
        ...
        with self.init_scope(): 
            ...
            #512 x 4 x 4
            self.rf1 = SPADEResBlock(opt, ngf * 8, ngf * 8)
            self.upf1 = define_upsampling(opt, ngf * 8)
            ...
            #512 x 32 x 32
            self.r3 = SPADEResBlock(opt, ngf * 8, ngf * 4)
            self.up3 = define_upsampling(opt, ngf * 4)

            #256 x 64 x 64
            self.r4 = SPADEResBlock(opt, ngf * 4, ngf * 2)
            self.up4 = define_upsampling(opt, ngf * 2)

            #128 x 128 x 128
            self.r5 = SPADEResBlock(opt, ngf * 2, ngf)
            self.up5 = define_upsampling(opt, ngf)

            #64 x 256 x 256
            self.r6 = SPADEResBlock(opt, ngf, ngf // 2)
