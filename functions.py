...
def adam_lr_poly(opt, trainer):
    ...
    max_iteration = trainer.stop_trigger.period
    
    threshold_iteration = max_iteration * opt.lr_poly_train_period
    if iteration < threshold_iteration:
        return

    iteration -= threshold_iteration
    max_iteration -= threshold_iteration

    glr = opt.g_lr * (1 - iteration / max_iteration)**opt.lr_poly_power
    dlr = opt.d_lr * (1 - iteration / max_iteration)**opt.lr_poly_power
